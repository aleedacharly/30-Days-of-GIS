...
The scatter plot shows r = 0.71 (p = 0.010), which is statistically significant. However, the relationship is positive rather than the expected negative — meaning districts with higher NDVI also tended to have higher LST in this dataset. This appears counterintuitive but reflects a confounding factor: several outer districts with high vegetation cover (Pankow, Reinickendorf) also have large impervious areas that drive up their LST, while some inner districts with low NDVI (Friedrichshain-Kreuzberg, Steglitz-Zehlendorf) happen to have lower mean LST due to their specific surface composition. This tells us that NDVI alone is insufficient to explain LST variation at district scale — land cover type, building density, albedo, and proximity to water bodies all contribute.
